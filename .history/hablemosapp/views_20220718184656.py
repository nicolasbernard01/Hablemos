    ...
    if user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if search != '':
                ...
                debates_search = Debate.objects.filter(titulo__icontains = search)

                ctx = {'debates_search':debates_search,'mensajes': mensajes}

                messages.success(request,r'Estos son los resultados de su busqueda')

                return render(request, r'hablemosapp/debates.html', ctx)

            else:
                
                messages.error(request, r'Su busqueda no ha arrojado resultados')

                return render(request, r'hablemosapp/debates.html', {'debates_search':debates_search, 'mensajes': mensajes})
