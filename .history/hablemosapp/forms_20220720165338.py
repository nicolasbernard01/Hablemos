...
class CrearDebateForm(forms.Form):
    ...
    titulo = forms.CharField(label = 'titulo', max_length=60, required=tr)
    subtitulo = forms.CharField(label='subtitulo', max_length=150)
    cuerpo = forms.Textarea(label='cuerpo')
    imagen = forms.ImageField('imagen',upload_to = 'PostImagen/',blank=True, null=True)
    creado = forms.DateTimeField(default=timezone.now)
