...
def debates(request):
    ...
    if user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            searcj = request.POST['search'] 

            if buscar != '':

                busqueda = Debate.objects.filter(titulo__icontains = buscar).values

                return render(request, r'hablemosapp/debates.html', {'busqueda':busqueda,'mensajes':mensajes})
