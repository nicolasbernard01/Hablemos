...
def debates(request):
    ...
    queryset = queryset.GET.get('search')

    if queryset:

        debates = Debate.objects.filter(titulo__icointains = queryset).values

        return render(request, r'hablemosapp/debates', {'debates':debates})
...
class DebateCreate(LoginRequiredMixin ,CreateView):
...
def buscar_debate(request):

    if request.POST == 'POST':

        search = request.POST['search']

        buscardebates = Debate.objects.filter(titulo__icontains = search)

        messages.success(request, r'Aqui estan los resultados de su busqueda')

        return render(request, r'hablemosapp/debates.html', {'buscardebates':buscardebates, 'search':True})
    
    else:

        messages.error(request, r'No hay coincidencias')
