...
def crear_debate(request):
    ...
    if request.method == 'POST':
        ...
        form = DebateForm(request.POST, request.FILE)

        if form.is_valid():

            info = form.cleaned_data

            nuevo_post = Debate(titulo=info['titulo'], subtitulo=info['subtitulo'],cuerpo=info['cuerpo'],imagen=info['imagen'], request.user=info['autor'])
