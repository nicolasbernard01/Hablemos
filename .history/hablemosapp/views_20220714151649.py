...
def editar_perfil(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, r"Su perfil ha sido actualizado exitosamente")

            return redirect('debate_list')

        else:

            if messages.error == 'password1' or 'password2':

                messages.error(request, r"Ha ingresado un password que no es valido")

            elif messages.error == 'email':

                messages.error(request, r"Por favor verifique su email")
